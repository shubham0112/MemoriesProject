...
export const getPosts = async (req, res) => {
    ...
    try {
        const LIMIT = 8; // max no. of posts to be displayed on one page
        const startIndex = (Number(page)-1)*LIMIT; //get the starting index of post for every page
...
export const likePost = async (req,res) => {
    ...
    // since it is first passing through auth middlewre it should have userId
    if(!req.userId){
    ...
    if(index===-1){
        // not liked already => like the post
        post.likes.push(req.userId);
    }else{
        // already liked => dislike the post
